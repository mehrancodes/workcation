<template lang="pug">
  div
    div(v-for='(location, i) in locations' :class="{ 'mt-10 sm:mt-6': i > 0 }")
      div(class='px-4 xl:px-8')
        h3.text-gray-900.text-xl {{ location.title }}
        p.text-gray-600 {{ location.description }}

      div(class='mt-2 sm:overflow-x-auto')
        div(class='px-4 sm:inline-flex sm:pt-2 sm:pb-8 xl:px-8')
          div(v-for='(property, i) in location.properties' :class="{'mt-10 sm:ml-4': i > 0 }" class='sm:mt-0 sm:w-80 sm:flex-shrink-0')
            wc-property-card(:property='property')
</template>

<script>
import WcPropertyCard from '@/components/PropertyCard'
export default {
  components: {
    WcPropertyCard
  },
